...
long int operand_1 = 0 , operand_2 = 0, result_non_div;
float result_div = 0;
unsigned char cur_oper = '\0', prev_oper = '\0', oper = '\0', result_str[15], input_num_chars = 0;
void main()
{
        unsigned char Key;
        char *str;        
			   ...
			   switch(Key)
                 ...
                 case '=':
                    ...
                    switch(oper)
                                        {
                                            case '+':
                                               result_non_div = operand_1 + operand_2;
                                            break;
                        case '-':
                                               result_non_div = operand_1 - operand_2;
                                            break;
                        case '*':
                             ...
                             case '/':
                                              if(operand_2)
											  {	  
											       result_div = (float)operand_1 / operand_2;
											  }
						                      else
                                              {  
                                                  calc_status.error_flag = 1;
									...
									if(!calc_status.error_flag)
									{
										if(oper == '/')
										{
                                           if(!FloatToStr(result_div, result_str))
									       {
										...
										{
											if(result_non_div > -32767L && result_non_div < 32767L )
											{
											    LongToStr(result_non_div, result_str);
									            str = ltrim(result_str);
									            Lcd_Out_Cp(str);
                                            }    
                                           else
